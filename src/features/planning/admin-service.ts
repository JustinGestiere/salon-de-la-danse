...
type LoadedSlot = Awaited<ReturnType<typeof loadSlot>>;
...
/// Écarts aux règles du planning, relus sous le verrou du bénévole comme pour
/// la réservation en libre-service : sans lui, une réservation simultanée du
/// bénévole échapperait au contrôle. Pas de contrôle du brouillon ici, la
/// régie peut modifier un planning validé.
async function collectRuleViolations(
  tx: Prisma.TransactionClient,
  { volunteerId, slot, rules }: RuleCheck,
): Promise<string[]> {
  await tx.$queryRaw`SELECT id FROM volunteer WHERE id = ${volunteerId} FOR UPDATE`;
  const current = await loadSelectedCells(tx, volunteerId);
  ...
  return [violation?.message, slot.isOpen ? null : "Cette case est fermée."].filter(
...
export async function assignVolunteerByAdmin(
  ...
  return db.$transaction(async (tx) => {
    const reasons = await collectRuleViolations(tx, { volunteerId: volunteer.id, slot, rules: actor.rules });
