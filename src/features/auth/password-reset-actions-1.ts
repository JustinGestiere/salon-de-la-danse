...
import {
  PASSWORD_RESET_EMAIL_COOLDOWN_MINUTES,
...
// Clé sur l'adresse saisie, qu'un compte existe ou non : le refus ne dit
// rien de plus sur les adresses inscrites.
const passwordResetEmailCooldown = createRateLimiter({
  maxAttempts: 1,
  windowMs: PASSWORD_RESET_EMAIL_COOLDOWN_MINUTES * MS_PER_MINUTE,
});
...
const COOLDOWN_MESSAGE = `Un lien vient déjà d'être demandé pour cette adresse. Réessayez dans ${PASSWORD_RESET_EMAIL_COOLDOWN_MINUTES} minutes.`;
...
export async function requestPasswordResetAction(input: unknown): Promise<ActionResult> {
  ...
  if (!passwordResetEmailCooldown.tryConsume(parsed.data.email.trim().toLowerCase())) {
    return fail("rateLimit.cooldown", COOLDOWN_MESSAGE);
  }
